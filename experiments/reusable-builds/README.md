...
Hosted Linux ARM64 tests and static-link checks pass; its highest glibc symbol
version is 2.34 (ceiling 2.35). macOS ARM64 passes the Metal regression suite,
installer checks and TLS fixture. A lockfile generated by that macOS job, with
Linux extension entries removed, successfully resolved `@cuda//:cublas_headers`
and analyzed the CUDA installation on Linux. The committed lockfile retains the
independently evaluated Linux AMD64 and macOS ARM64 results for both extensions.

All four platform jobs pass at `c713635` in
[the platform regression run](https://github.com/pawelchcki/llm-cc/actions/runs/34508790474).
Windows passes 14 applicable tests (10 platform-inapplicable tests skipped),
including Schannel trust/hostname/redirect/resume verification, and links the
distribution executable. The final local CUDA consumer check also passes with
`compute_86:sm_86`: the renamed module installs without `_main`, ignores the
consumer's conflicting Git identity and verifies the installed bundle offline.
That check reused the independently initialized consumer's CPU/download cache;
it is functional regression evidence, separate from the controlled benchmarks.
