...
#   wget -qO- https://github.com/pawelchcki/llm-cc/releases/latest/download/install.sh | sh
#
# Options may follow "sh -s --":
...
# LLM_CC_RELEASE_BASE (an alternative release directory, mainly for tests).
# Model weights are never installed; llm-cc downloads them on first use.
set -eu
...
"$work/$asset" --version >/dev/null || fail "downloaded executable does not run on this host"
