...
constexpr std::string_view kUsageBeforeContext =
    "Usage:\n"
    "  llm-cc SOURCE [--lang rust|c|cpp] [--model GGUF] [OPTIONS]\n"
...
std::string ScoreSource(const AnalyzeArguments& arguments,
...
int RunAnalyze(const AnalyzeArguments& arguments) {
  ...
  const auto model = llmcc::ResolveModel(
      ...
      cache_dir, llmcc::DownloadDefaultModel);
  ...
  const std::string jsonl =
      ScoreSource(arguments, cache_dir, model, preprocessed);
