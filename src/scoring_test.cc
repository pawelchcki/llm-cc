...
double EntropyOrNan(const llmcc::TokenScore& score) {
...
int main() {
  ...
  const llmcc::TokenScore uniform = llmcc::ScoreToken(uniform_logits, 0, true);
  Expect(std::abs(EntropyOrNan(uniform) - std::log(4.0)) < 1e-12,
         "uniform entropy");
  Expect(!llmcc::ScoreToken(logits, 2).entropy.has_value(),
         "entropy not requested");

  const std::vector<float> singleton_logits = {42.0F};
  const llmcc::TokenScore singleton =
      llmcc::ScoreToken(singleton_logits, 0, true);
  ...
  const llmcc::TokenScore large = llmcc::ScoreToken(large_logits, 0, true);
  ...
  const llmcc::TokenScore masked = llmcc::ScoreToken(masked_logits, 1, true);
  ...
  const llmcc::TokenScore masked_uniform =
      llmcc::ScoreToken(uniform_after_masking, 2, true);
  ...
  Expect(llmcc::BytesToHex(std::string("A\0", 2)) == "4100", "hex encoding");
  Expect(llmcc::JsonEscapeBytes("a\n\"b") == "a\\n\\\"b", "JSON escaping");
  Expect(llmcc::JsonEscapeBytes("\xf0\x9f\x98\x80") == "\xf0\x9f\x98\x80",
         "valid UTF-8 preservation");
  Expect(llmcc::JsonEscapeBytes("\x80") == "\\ufffd",
